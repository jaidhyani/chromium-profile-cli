...
from chromium_profile_cli.leveldb import iter_leveldb_records
...
class LocalReader:
    ...
    def get_tabs(self) -> list[Device]:
        ...
        try:
            records = iter_leveldb_records(temp_leveldb)
        except Exception:
            return []

        # LevelDB contains all records including old/deleted ones.
        # Deduplicate by keeping only the last occurrence of each key.
        kv_pairs: dict[bytes, bytes] = {}
        for record in records:
            kv_pairs[record.user_key] = record.value
